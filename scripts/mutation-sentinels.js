...
const mutations = [
  ...
  { name: 'interrupted-write-recovery', file: 'src/domain/engine.js', from: 'this.store.listDaysByStatuses([DayStatus.POSTING, DayStatus.OUTCOME_UNKNOWN, DayStatus.CORRECTING])', to: 'this.store.listDaysByStatuses([DayStatus.OUTCOME_UNKNOWN, DayStatus.CORRECTING])' },
  { name: 'interrupted-correction-recovery', file: 'src/domain/engine.js', from: 'if (day.status === DayStatus.CORRECTING) {', to: 'if (false && day.status === DayStatus.CORRECTING) {' },
