...
export function setup({ market = 'US', database = ':memory:', paid = true, departmentRef = '' } = {}) {
  ...
  store.createLocation({ id: source.locationId, workspaceId: source.workspaceId, sourceLocationKey: `csv-${source.locationId}`, timezone: source.timezone, departmentRef });
  store.saveMapping(source.locationId, mapping);
  store.putEntitlement(source.workspaceId, paid
    ? { status: 'PAID', postingPaused: false }
    : { status: 'NOT_STARTED', postingPaused: false });
